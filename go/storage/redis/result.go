...
import (
	...
	"github.com/michaelhendraw/library/go/convert"
)
...
func (r *Result) ScanResult() (nextCursor int, keys []string, err error) {
	reply, err := rgo.Values(r.Value, r.Error)
	if err != nil {
		return
	}

	_, err = rgo.Scan(reply, &nextCursor, &keys)
	return
}
